...
def available_times(arg):
    ...
    for hour in range(start_hour, end_hour + 1):
        for minute in ['00', '30']:
            if hour == start_hour and minute == '00':
                if start_minute == 30:
                    continue
            elif hour == end_hour and minute == '30':
                if end_minute == 0:
                    continue
...
def change_working_hours():
    if current_user.is_authenticated:
        if request.method == "GET":
            ...
            conn.close()
            print(working_hours)
            return render_template("change_working_hours.html", working_hours=working_hours)
            ...
            conn = sqlite3.connect('database/database.db')
            cursor = conn.cursor()
            cursor.execute('UPDATE working_hours SET start_time = ? WHERE day = ?', (new_start_time, day))
            conn.commit()
            conn.close()

            flash('Start time updated successfully', 'success')
            return redirect(url_for('change_working_hours'))
    else:
        return redirect(url_for('login'))


@app.route('/admin/edit_working_hours', methods=['POST'])

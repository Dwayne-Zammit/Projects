...
def book_appointment():
    ...
    if request.method == "GET":
        ...
        conn.close()
        # Render the HTML template with the form, taken times, and services
        return render_template('appointment_form.html', form=form, takenTimes=taken_times, services=services)
       
    if request.method == "POST":
        # Retrieve the selected service from the form
        ...
        cursor.execute(f"INSERT INTO appointments (datetime, name, email, phone, service) VALUES ('{str(formatted_date + ' ' + time)}', '{name}', '{email}', {phone}, '{service}')")
        conn.commit()
        conn.close()
...
def get_services():
    ...
    services = [{'id': row[0], 'service': row[1]} for row in cursor.fetchall()]
    conn.close()
    return jsonify({'services': services})

@app.route('/service_cost/<service_id>')
def get_service_cost(service_id):
    ...
    service_cost = result[0] if result else None

    return jsonify({'serviceCost': service_cost})
    ...
    for hour in range(start_hour, end_hour + 1):
        for minute in range(0, 60, 15):
            if hour == start_hour and minute < start_minute:
                continue
            elif hour == end_hour and minute > end_minute:
                continue
...
# @app.route('/admin/change_working_hours', methods=['GET', 'POST'])
# def change_working_hours():
#     if current_user.is_authenticated:
#         if request.method == "GET":
#             return render_template("change_working_hours.html")
#         elif request.method == "POST":
#             start_time = request.form.get("end_time")
#             end_time = request.form.get("end_time")
#             day = request.form.get("end_time")
#             conn = sqlite3.connect('database/database.db')
#             cursor = conn.cursor()
#             cursor.execute(f"ALTER TABLE working_hours set start_time = '{start_time}', end_time = '{end_time}' where day = '{day}'")
#             conn.commit()
#     else:
#         return redirect(url_for('login'))
        ...
        if request.method == "GET":
            ...
            for day in days_order:
                for line in working_hours:
                    if line[1] == day:
                        days_ordered.append(line)          
            return render_template("change_working_hours.html", working_hours=days_ordered)

    else:
        return redirect(url_for('login'))


@app.route('/admin/edit_working_hours/', methods=['GET','POST'])
def edit_working_hours():
    if current_user.is_authenticated:
        id = request.args.get("id")
        if request.method == "GET":

            # Retrieve the working hours data for the specified id from the database
            conn = sqlite3.connect('database/database.db')
            cursor = conn.cursor()
            cursor.execute(f"SELECT * FROM working_hours WHERE id = ?", (id,))
            working_hours = cursor.fetchone()
            conn.close()

            if working_hours:
                return render_template("edit_working_hours.html", working_hours=working_hours)
            else:
                flash('Invalid working hours ID', 'error')
                return redirect(url_for('change_working_hours'))
        elif request.method == "POST":
            ...
            conn = sqlite3.connect('database/database.db')
            cursor = conn.cursor()
            cursor.execute(f"UPDATE working_hours SET start_time = '{start_time}' WHERE id={id}")
            # conn.commit()
            cursor.execute(f"UPDATE working_hours SET end_time = '{end_time}' WHERE id={id}")
            conn.commit()

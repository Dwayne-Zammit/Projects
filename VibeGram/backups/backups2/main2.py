...
def home():
    ...
    return render_template('home.html', user_does_not_exist=False)
...
def unfollow_user(profile_username):
    if request.method == "POST":
        ...
        conn.commit()
        global last_unfollowed_user
        last_unfollowed_user = profile_username
        # return(f"You: {username} will follow {profile_username}")
        return redirect(url_for('search_profile'))
